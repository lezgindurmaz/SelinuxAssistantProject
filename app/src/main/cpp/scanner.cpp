...
ScanResult Scanner::scanFile(const std::string& filePath) {
    ...
    if (m_config.useLocalDB) {
        LocalDB localDB("/data/data/com.yourapp/files/signatures.db");
        if (localDB.open()) {
    ...
    if (m_config.useCloudLookup) {
        ...
        auto response = cloud.lookup(result.hashes.sha256, result.hashes.md5);
        if (!response.networkError && response.found) {
