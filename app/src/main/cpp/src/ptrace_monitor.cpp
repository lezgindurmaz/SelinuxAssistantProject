...
bool BehavioralAnalyzer::readSyscallEntry(pid_t pid, SyscallEvent& ev) {
    ...
    if (m_isArm64) {
        ...
        // ARM32: PTRACE_GETREGS
        Arm32Regs regs{};
        struct iovec iov = { &regs, sizeof(regs) }; if (ptrace(PTRACE_GETREGSET, pid, reinterpret_cast<void*>(NT_PRSTATUS), &iov) != 0)
            return false;
...
bool BehavioralAnalyzer::readProcSyscall(pid_t pid, SyscallEvent& ev) {
    ...
    int parsed = sscanf(buf, "%lu %lx %lx %lx %lx %lx %lx %lx %lx",
